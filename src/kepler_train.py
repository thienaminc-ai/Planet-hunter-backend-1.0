...
def train_kepler_model(data_path, model_name, output_dir=None, param_grid=None):
    """
    Huấn luyện mô hình Random Forest trên dữ liệu Kepler.
    - Input: 
        - data_path: Đường dẫn file CSV đã xử lý.
        - model_name: Tên mô hình để lưu (ví dụ: 'kepler_model1').
    ...
    logger.info("Starting Kepler model training...")
    ...
    if 'koi_disposition' not in df.columns:
        logger.error("Target column 'koi_disposition' not found.")
        raise ValueError("Target column 'koi_disposition' not found in dataset.")

    # Tách features và target
    X = df.drop(['koi_disposition', 'kepid'], axis=1, errors='ignore')
